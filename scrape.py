...
try:
    ...
    show_all_link = wait.until(
        ec.element_to_be_clickable((By.XPATH, '//a[text()="show all 215 participants"]'))
    )
    show_all_link.click()
    ...
    table = wait.until(
        ec.presence_of_element_located(
            (By.CSS_SELECTOR, "table.MainTable.tablesorter.tablesorter-default")
        )
    )
    ...
    for row in result_rows:
        ...
        results = {
            summary_keys[i]: row_data[i]
            for i in range(len(row_data))
            if i not in ignore_summary_indices
        }
        ...
        if len(row_data) > 1:
            row.click()
            details = wait.until(
                ec.presence_of_element_located((By.CLASS_NAME, "divDetailsResults"))
            )
            ...
            for lap_detail in details_rows[1:-1]:  # ignore 1st and last row
                ...
                lap_details_data = [
                    lap_detail_column.text for lap_detail_column in lap_detail_columns
                ]
                lap = {
                    lap_keys[i]: lap_details_data[i]
                    for i in range(len(lap_details_data))
                    if i not in ignore_lap_indices
                }
                if lap["lap_time"] != " ":
                    laps.append(lap)

            results["lap_details"] = laps
            close_details = wait.until(
                ec.element_to_be_clickable((By.CLASS_NAME, "imgDetailsClose"))
            )
            close_details.click()
            time.sleep(1)
        ...
        if results:
            print(
                f"place={results['place']}, laps={results['laps']}, lap_details_count={len(results['lap_details'])}"
            )
            data.append(results)
